...
def request_paper_with_arXiv_api(keyword: str, max_results: int, link: str = "OR", raw_query: str = None) -> List[Dict[str, str]]:
    # keyword = keyword.replace(" ", "+")

    if raw_query is not None:
        url = f"http://export.arxiv.org/api/query?search_query={raw_query}&max_results={max_results}&sortBy=lastUpdatedDate"
    else:
...
def get_daily_papers_by_keyword_with_retries(keyword: str, column_names: List[str], max_result: int, link: str = "OR", retries: int = 6, raw_query: str = None) -> List[Dict[str, str]]:
    for _ in range(retries):
        papers = get_daily_papers_by_keyword(keyword, column_names, max_result, link, raw_query)
...
def get_daily_papers_by_keyword(keyword: str, column_names: List[str], max_result: int, link: str = "OR", raw_query: str = None) -> List[Dict[str, str]]:
    # get papers
    papers = request_paper_with_arXiv_api(keyword, max_result, link, raw_query) # NOTE default columns: Title, Authors, Abstract, Link, Tags, Comment, Date
